...
def computePCA(img):
    ...
    print("th=" + str(th) + ";phi=" + str(phi))
    print("th=" + str(res_pca[0]) + ";phi=" + str(res_pca[1]))
    print("###########################################################")
...
def randomvariate(n=1000, xmin=0, xmax=1, s=1.3):
    xminHelp = xmin
    xmin = 0
    x = np.linspace(xmin, xmax, 1000)
    y = ro(x, s)
    y[x <= xminHelp] = ro(xminHelp, s)
    pmin = 0.
    pmax = y.max()

    ran = np.zeros((n))
    ran = generate_sample(ran, xmin, xmax, pmin, pmax, s)
    return ran

@njit(parallel=True)
def generate_sample(ran, xmin, xmax, pmin, pmax, s):
...
count_max = 41 * 90
count = 0
for th in range(0, 82, 2):
    str(str(th) + " of 82")
    for phi in range(0, 180, 2):
        foo2 = randomvariate(N, xmin=.5, xmax=5 * r0, s=s)
        img = generateArray(foo2, th, phi, hitX, hitY, offsetX, offsetY)
        # set your path here
        with open('d:\\dane\\credo_showers\\numpy\\hit_th=' + str(th) + '_phi=' + str(phi) + '.npy', 'wb') as f:
            np.save(f, img)

        end = time.time()
        count = count + 1
        print("time=" + str(end - start) + " " + str(count) + " of " + str(count_max))
